...
public class UserTaskServiceImpl extends CommonServiceImpl<UserTask> implements UserTaskService {
    ...
    public Boolean addUserTask(String userId, Task task, Date assignTime) {
        ...
        userTask.setDiscription(task.getRemark());
    ...
    public Boolean doTask(String userId, UserTask userTask) throws Exception {
        ...
        logger.error("user======================================");
        if (user == null) {
            logger.error("user == null=====================================false");
            return false;
        }
        if (user.getRemainTaskNo() <= 0) {
            logger.error("user.getRemainTaskNo() <= 0=====================================false");
            return false;
        }
        if (TaskStatusType.PENDING.getCode().intValue() != userTask.getStatus().intValue()) {
            logger.error("TaskStatusType.PENDING.getCode() != userTask.getStatus()=====================================false");
            return false;
        }
        if (userTask.getCreateTime().getTime() > System.currentTimeMillis()) {
            logger.error("userTask.getCreateTime().getTime() > System.currentTimeMillis()=====================================false");
            return false;
        }
        logger.error("=====================================================");
        if (user.getRemainTaskNo() == 1) {
            logger.error("getRemainTaskNo() == 1======================================");
            //冻结用户账号，等待下次激活
            userService.updateUserStatus(userId, UserStatusType.OUT.getCode());
            userService.weeklyIncomeAmt(user);
        }
        logger.error("===================updteModel  到此一游==================================");
    ...
    public static void main(String[] args) {
        ...
        Calendar calendar=Calendar.getInstance();
        calendar.setTime(DateTimeUtil.parseDate("2017-09-25 09:25:22",DateTimeUtil.PATTERN_LONG));
