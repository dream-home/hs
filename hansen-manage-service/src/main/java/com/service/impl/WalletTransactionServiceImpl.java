...
//                break;
    ...
    public Boolean checkCoinOut(SysUser loginUser, String orderId,Integer status) throws Exception {
        ...
        if (ToolUtil.isEmpty(order.getUserId())){
            return false;
        }
        User con = new User();
        ...
        BitcoinClient client = WalletUtil.getBitCoinClient(currencyType);
        Double confirAmt=order.getAmount()-order.getFee();
        String txtId = WalletUtil.sendToAddress(client, order.getAddress(), new BigDecimal(confirAmt.toString()), "用户" + order.getUserId() + "提币", "用户" + order.getAddress() + "收币");
